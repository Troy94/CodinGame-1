...
int main(){
    ...
    while(1){
		...
		point defend = point(  myID*16000, 3750);
		...
		if(obj[wiz1].state==0){
			...
			if(distfromgoal<4000){
				thrust = 500.0f;
				printf("THROW %.0f %.0f %0.f GOOOAL !! \n", goal.x, goal.y, thrust );
			}
			else{
		...
		if(obj[wiz2].state==0){
			...
			if(defend.distance(obj[snaffle].loc) < 2000 && mana >=20){
				DB("Wizard 2 pulling snaffle %d\n", obj[snaffle].id);
				printf("ACCIO %d\n", obj[snaffle].id );
				mana-=20;
			}
			else{
